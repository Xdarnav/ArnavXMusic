...
from ArnavX import YouTube, app
from ArnavX.core.call import Arnav
from ArnavX.misc import SUDOERS, db
from ArnavX.utils.database import (is_active_chat,
                                       is_music_playing, set_loop,
                                       is_nonadmin_chat, music_off,
                                       music_on)
from ArnavX.utils.decorators.language import languageCB
from ArnavX.utils.formatters import seconds_to_min
from ArnavX.utils.inline.play import (panel_markup_1,
                                          stream_markup,
                                          telegram_markup)
from ArnavX.utils.stream.autoclear import auto_clean
from ArnavX.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Arnav.pause_stream(chat_id)
        ...
        await Arnav.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Arnav.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Arnav.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Arnav.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
